...
def Update_statement(selected_stock):
    ...
    statement_data['Porfolio value'].append(portfolio_data['Market Value'][idx])
    statement_data['NAV'].append(portfolio_data['Market Value'][idx]/portfolio_data['Actual Vol'][idx])
...
if prev_summary_df is not None:
    ...
    if 'Number of wins' in prev_summary_df.columns:
        ...
        if not count_win_series.empty:
            # เข้าถึงค่าแรกของคอลัมน์
            last_count_win = count_win_series.iloc[0]
    else:
        count_win = 0
    ...
    if 'Number of matched trades' in prev_summary_df.columns:
        ...
        if not count_sell_series.empty:
            # เข้าถึงค่าแรกของคอลัมน์
            last_count_sell = count_sell_series.iloc[0]
    else:
        count_sell = 0
...
if prev_portfolio_df is not None:
    ...
    portfolio_data['Start Vol'] = portfolio_data['Actual Vol']    
else:
    pass
...
if prev_statement_df is not None:
    ...
    statement_data['Start Vol'] = statement_data['Actual Vol']    
else:
    pass


csv_file = "~/Desktop/Daily_Ticks.csv"  # ชื่อไฟล์ CSV ของคุณ
